...
contract CrossChainNameServiceRegister is OwnerIsCreator {
    ...
    function enableChain(uint64 chainSelector, address ccnsReceiverAddress, uint256 gasLimit) external onlyOwner {
        s_chains.push(
            Chain({chainSelector: chainSelector, ccnsReceiverAddress: ccnsReceiverAddress, gasLimit: gasLimit})
        );
    }
    ...
    function register(string memory _name) external {
        ...
        for (uint256 i; i < length;) {
            ...
            Client.EVM2AnyMessage memory message = Client.EVM2AnyMessage({
                ...
                extraArgs: Client._argsToBytes(Client.EVMExtraArgsV1({gasLimit: currentChain.gasLimit})),
